...
import { CombatState, drawCard } from "../combatSlice";
...
export const Battle = (props: IBattleProps) => {
  ...
  */
  ...
  const { topTeam, bottomTeam, currentTurn, currentTurnPhase } = useSelector(
    (store: any) => store.battle
  ) as CombatState;

  // Can still use with an useEffect!

  // Use: dispatch(action()) to dispatch actions to the store
  ...
  return (
    <>
      ...
      ></Battlefield>
      <div>{topTeamCombatants[0]?.test}</div>
